...
input <- input%>% dplyr::select(out_first_long_covid_date, out_covid_date, sub_cat_covid_phenotype) %>%
...
function_summary_histogram_days <- function(input, subgroup){
  input <- input%>%filter(sub_cat_covid_phenotype== subgroup)
  ...
  figure_hist<-ggplot(as.data.frame(days), aes(x=days)) +
    ...
    scale_x_continuous(breaks = seq(0, 800, by = 50), limits = c(0,800)) +
