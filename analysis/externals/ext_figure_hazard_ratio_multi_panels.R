...
for(i in 1:length(df_list)){
  analysis <- sub("hazard_ratio_estimates_", "", file_list[i])
  analysis <- sub(".csv","", analysis)
  analysis_col = rep(analysis, nrow(df_list[[i]]))
  a <- data.frame(df_list[i])
  a <- data.frame(a,analysis_col)
  df_list[[i]] <- a
}
...
data <- data%>%filter(!grepl("selected",data$analysis_col))
data <- data%>%filter(!grepl("infected",data$analysis_col))
table(data$analysis_col)
...
# slightly different
  ...
  hr <- data
  hr <- hr %>% 
    dplyr::select(c("term","hazard_ratio", "conf.low", "conf.high", "std.error", "analysis_col")) %>%
  ...
  df <- as.data.frame(lapply(df, rep, 4))
  df$analysis_col = c(rep("full_all", nrow(df)/4),
                      rep("full_all_vax_c", nrow(df)/4),
                      rep("full_all_vax_td", nrow(df)/4),
                      rep("full_all_vaccinated", nrow(df)/4))
